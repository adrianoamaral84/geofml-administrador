...
<form method="GET" action="{{ route('user.list.inativos') }}" class="form-inline">
    <div class="input-group input-group-sm">
        ...
        @if(request('search'))
            <div class="input-group-append ml-2">
                <a href="{{ route('user.list.inativos') }}" class="btn btn-secondary btn-sm">Limpar</a>
            </div>
        @endif
    </div>
</form>
                                ...
                                <table class="table table-striped table-bordered table-hover flip-content" id="tabela">
                                    ...
                                    <tbody>   
                                            ...
                                            <tr>
                                                ...
                                                <td> 
                                                    @if($item->status == 4)
                                                        <a href="{{ route('hospedagem.verdados_aguardando_liberacao', ['id' => Crypt::encrypt($item->id)]) }}" title="Ver Dados"> {{$item->user->name}}</a>
                                                    @elseif($item->status == 0 or $item->status == 1 or $item->status == 2 or $item->status == 3 or $item->status == 5 or $item->status == 6 or $item->status == 7)
                                                ...
                                                <td style="text-align: center;">
                                                    ...
                                                    @if($item->status == 2) @if(isset($item->undHB->sigla))<b>{{ $item->undHB->sigla }}{{$item->undHB->classe->classe}}</b> - @endif @endif{{ $item->tipouh->descricao }}
