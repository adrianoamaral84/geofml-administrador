...
<article class="items-list-page">
    <div class="title-search-block">
        <div class="title-block">
            <div class="row">
                <div class="col-md-6">
                    <h3 class="title">Relatório de Hospedados Pagos Mensal {{$mes}}/{{$ano}}</h3> 
                    <small>Lista as hospedagens do mês e os valores arrecadados!</small>
                </div>
            </div>
        </div>
        <div class="items-search">

        <!-- <form method="POST" action="{{ url('admin/relatorio/arrecadacaoview') }}" class="form-inline mb-3">
    @csrf
    ...
    <div class="input-group input-group-sm">
        ...
        @if($search)
            <div class="input-group-append ml-2">
                <button type="submit" class="btn btn-secondary" onclick="document.getElementsByName('search')[0].value=''">
                    Limpar
                </button>
            </div>
        @endif
...
<form method="POST" action="{{ url('admin/relatorio/arrecadacao/view') }}" class="form-inline mb-3">
    @csrf
    <input type="hidden" name="ano" value="{{ $ano }}">
    <input type="hidden" name="mes" value="{{ $mes }}">
                                ...
                                <table class="table table-striped table-bordered table-hover flip-content" id="tabela">
                                    <thead class="flip-header">
                                        <tr>
                                            ...
                                            <th width="" style="text-align: center;">Nome</th>
                                            <th width="10%" style="text-align: center;">Check-In</th>
                                            <th width="10%" style="text-align: center;">Check-Out</th>
                                            <th width="6%" style="text-align: center;">Diárias</th>
                                            <th width="12%" style="text-align: center;">Valor Diária</th>       
                                            <th width="12%" style="text-align: center;">R$</th>
                                    ...
                                    <tbody>  
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @if($libera == 1)
                                                            <a href="{{ route('relatorio.verdados', ['id' => Crypt::encrypt($item->id)]) }}" title="Ver Dados">{{$item->name}}</a>
                                                    @else
                                                            {{$item->name}}
                                                    @endif
                                               ...
                                               <td style="text-align:center">
                                                @if(empty($item->checkin_at))

                                                @else
                                                   {{ \Carbon\Carbon::parse($item->checkin_at)->format('d/m/Y H:i:s') }}   
                                                @endif
                                                </td>
                                               <td style="text-align:center">
                                                @if(empty($item->checkout_at))
                                                
                                                @else
                                                   {{ \Carbon\Carbon::parse($item->checkout_at)->format('d/m/Y H:i:s') }}    
                                                @endif
                                                </td>
                                               <td style="text-align:center">
                                                @if(empty($item->checkout_at))
                                               ...
                                               <td style="text-align:center">
                                                      R$ {{ number_format($item->valortarifa, 2, ',', '.' ) }}
                                                </td>
                                               <td style="text-align:center">
                                                @if(empty($item->checkout_at))
                                                R$ {{ number_format($item->valortarifa, 2, ',', '.' ) }}
                                    ...
                                    <tr>
                                        ...
                                        <th style="text-align:center"> R$ {{ number_format( $valor, 2, ',', '.' )}}</th>
                                    </tr>
        </tfoot>
                                </table>
