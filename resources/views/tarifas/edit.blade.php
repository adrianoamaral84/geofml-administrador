...
<article class="items-list-page">
    <div class="title-search-block">
        <div class="title-block">
            <div class="row">
                <div class="col-md-6">
                    <h3 class="title"> Editar Tarifas </h3>
                </div>
            </div>
        </div>
                    ...
                    <div class="card-block">
                        <section class="example">
                            <div class="table-flip-scroll">
                                <form id="password-form" action="{{ route('tarifas.update') }}" method="POST">
                                    @csrf
        ...
        <div class="row has-error">
            <div class="form-group col-sm-6 col-md-6 col-lg-6">
                            <label class="control-label">{{ __('Tipo Unidade Habitacional') }}</label>
                                ...
                                <input type="hidden" value="{{ $consulta->id }}" name="id" id="id" required maxlength="50">
                                ...
                                <select name="habitacao" id="habitacao" required class="custom-select mr-sm-2 @error('habitacao') is-invalid @enderror" autocomplete="off">
                                    ...
                                    <option value="{{$habitacao->id}}" @if($consulta->tipoundhab_id == $habitacao->id) selected @endif>
            ...
            <div class="form-group col-sm-6 col-md-6 col-lg-6">
                            <label class="control-label">{{ __('Grupo de Tarifas') }}</label>
                                ...
                                <select name="destinacao" id="destinacao" required class="custom-select mr-sm-2 @error('destinacao') is-invalid @enderror" autocomplete="off">
                                     <option value="">Grupo Tarifas</option>
                                
                                    @foreach($destinacoes as $destinacao)

                                         <option value="{{$destinacao->id}}" @if($consulta->grupo_destinacao_id == $destinacao->id) selected @endif>{{$destinacao->descricao}}</option>
                        ...
                        <div class="row has-error">
                            ...
                            <input type="text" class="form-control boxed @error('valor_baixa') is-invalid @enderror" value="{{ number_format($consulta->valor_baixa, 2, ',', '.')}}" name="valor_baixa" id="valor_baixa" autofocus required maxlength="50" onpaste="return false;">
                            @error('valor_baixa')
                                <span class="has-error" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <input type="text" class="form-control boxed @error('valor') is-invalid @enderror" value="{{ number_format($consulta->valor, 2, ',', '.')}}" name="valor" id="valor" autofocus required maxlength="50" onpaste="return false;">
                            @error('valor')
                                <span class="has-error" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
