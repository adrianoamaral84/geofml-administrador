...
@if($ultimoPagamento)
    <div class="alert alert-{{ $alert }}">
        ...
        @if($ultimoPagamento->data_situacao)
            <br>
            <small>
                Última atualização:
                {{ \Carbon\Carbon::parse($ultimoPagamento->data_situacao)->format('d/m/Y H:i') }}
            </small>
        @endif
    </div>
@endif
                        ...
                        <div class="col-sm-12 col-xl-12">
                            ...
                            <a href="javascript:;" data-target="#AprovaModal" data-toggle="modal" onclick="aprovapedido('{{Crypt::encrypt($hospedagem->id)}}')" title="Ver Aguardando Liberação" class="btn btn-success btn-xl" style="color: #fff">
                                ...
                                <div class="modal fade" id="ModalVoltarCancelar">
                                    <div class="modal-dialog" role="document">
                                    <form id="voltarcancelar" action="{{ route('hospedagem.retornarEnvioComprovante') }}" method="POST" enctype="multipart/form-data">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h4 class="modal-title"><i class="fa fa-warning"></i> Atenção</h4>
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript">
     ...
     function VoltarReserva(id)
    {
        ...
        var id = id;
        var url = '{{ route("hospedagem.retornarEnvioComprovante") }}';
        url = url.replace(':id', id);
