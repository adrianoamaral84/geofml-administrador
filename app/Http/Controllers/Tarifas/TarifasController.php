...
class TarifasController extends Controller
{
    ...
    public function index(){
   	$consulta = \App\Tarifas::paginate(20);
   	return view('tarifas.index', compact('consulta'));
   }
   ...
   /*public function edit($id){
    ...
    // app/Http/Controllers/Tarifas/TarifasController.php
    ...
    // CORREÇÃO: Usar findOrFail. Se não encontrar, o Laravel mostra um erro 404
    // ao invés de enviar NULL para a View.
    $consulta = \App\UnidadeHabitacional::findOrFail($id);
    
    return view('unidadehabitacional.edit', compact('consulta', 'tipos', 'classes', 'destinos'));
    }*/
    
    public function edit($id_criptografado) // Use um nome diferente para o parâmetro para clareza
{
    // 1. Tenta descriptografar
    try {
        $id = Crypt::decrypt($id_criptografado);
    } catch (\Illuminate\Contracts\Encryption\DecryptException $e) {
        // Se a descriptografia falhar, pare aqui e veja o erro!
        dd('Erro de Descriptografia. ID Criptografado:', $id_criptografado, 'Erro:', $e->getMessage());
    }

    // 2. Tenta buscar o objeto
    $consulta = \App\UnidadeHabitacional::find($id);

    // 3. Verifica o que foi encontrado e para a execução
    dd('ID Descriptografado:', $id, 'Resultado da Busca:', $consulta);
    
    // O restante do seu código (com o findOrFail, se você já o aplicou)
    /*
    $tipos = \App\TipoUndHab::all();
    $classes = \App\ClasseHabitacional::all(); 
    $destinos = \App\GrupoDestinacao::all();
    // $consulta = UnidadeHabitacional::find($id); // Linha original (com find)
    $consulta = \App\UnidadeHabitacional::findOrFail($id); // Linha corrigida (com findOrFail)
