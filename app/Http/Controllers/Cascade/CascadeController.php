...
class CascadeController extends Controller
{
    ...
    public function carregarUnidadesHabtacionais($id, $tipo){
        
        if($tipo == 11 or $tipo == 12){
        
             $listaUnidades = \App\UnidadeHabitacional::where('grupo_destinacao_id', $id)
            ->where('tipo_und_hab_id', $tipo)
            ->where('disponivel', 1)->with('tipohabitacao')->get();
