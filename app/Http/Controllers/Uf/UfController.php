...
{   
 ...
 public function index(Request $request)
{
    // Verificações de permissão que já existem no seu arquivo
    if(!\Auth::user()->hasRole('administrador_geral')){
        abort(403, "Desculpa, você não tem autorização!");
    }

    // Use o método input() ou get() para evitar erro de propriedade inexistente
    $search = $request->input('search');

    // Iniciamos a query. Certifique-se que \App\Uf é o caminho correto da sua Model
    $query = \App\Uf::query();
    ...
    // Retorna para a view garantindo que todas as variáveis esperadas existam
    return view('uf.index', compact('menuAtivo', 'search', 'consulta'));
}
