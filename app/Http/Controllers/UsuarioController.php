...
// dd($request->all());
    ...
    // ** INÍCIO DA CORREÇÃO: Lógica de Pesquisa Insensível a Acentos **
    $search = $request->input('pesquisa', ''); // Pega o termo de pesquisa
    //$search = $request->input('search', ''); // Pega o termo de pesquisa
    //$search = $request->input('busca', ''); // Pega o termo de pesquisa


    $query = \App\User::where('status', 1); // Começa a query base (status 1)
// UsuarioController.php (Aproximadamente Linha 88)

    if (!empty($search)) {
        $termo = $search;
        
        // Remove caracteres de formatação (ponto, traço) do CPF digitado pelo usuário
        // para buscar no banco (assumindo que o CPF está armazenado sem formatação)
        $termo_limpo = preg_replace('/[^0-9]/', '', $termo);

       // Verifica se o termo limpo é numérico e tem comprimento suficiente para ser um CPF (pelo menos 5 dígitos)
        // Isso nos permite decidir se a busca é por NOME ou CPF, contornando o bug do OR com COLLATE.
        if (is_numeric($termo_limpo) && strlen($termo_limpo) >= 5) {
            
            // Se for numérico, assume que é uma busca por CPF
            $query->where('cpf', 'LIKE', '%' . $termo_limpo . '%');
            
        } else {
            
            // Se for texto (Nome), aplica a busca insensível a acentos (COLLATE)
            // Esta cláusula AGORA FUNCIONARÁ, pois não está dentro de um OR agrupado que a estava quebrando
            $query->whereRaw("name COLLATE utf8_general_ci LIKE ?", ['%' . $termo . '%']);
        }
    }
    ...
    if(Laratrust::hasRole('administrador_especial')) {
        ...
        // Garantindo que a variável $search seja passada para repopular o campo
        return view('usuario.consulta', compact('menuAtivo', 'consulta', 'search'));
...
//dd("Estou usando a nova lógica de pesquisa!");
 // ADICIONE ESTA LINHA TEMPORARIAMENTE PARA TESTAR
 //dd($query->toSql()); // Isso irá mostrar o comando SQL que está sendo executado
//dd($query->toSql(), $query->getBindings());
        
        
        $consulta = $query->select('id', 'name', 'cpf', 'status')
        //->with('roles')
        ...
        //$consulta = User::whereRoleIs('administrador_geral')->get();
        //dd('Adm geral');
        return view('usuario.consulta', compact('consulta', 'search')); // Passando $search
        ...
        // Mantendo a lógica original (whereRoleIs) e adicionando o filtro de pesquisa, se houver
        $consulta = User::whereRoleIs('atendente')
            ->when(!empty($search), function ($q) use ($search) {
                $q->whereRaw("name COLLATE utf8_general_ci LIKE ?", ['%' . $search . '%']);
            })
            ->paginate(20);
        ...
        // Mantendo a lógica original (whereRoleIs) e adicionando o filtro de pesquisa, se houver
        $consulta = User::whereRoleIs('hospede')
            ->when(!empty($search), function ($q) use ($search) {
                $q->whereRaw("name COLLATE utf8_general_ci LIKE ?", ['%' . $search . '%']);
            })
            ->get();
        ...
        $consulta = $query->select('id', 'name', 'cpf', 'status')
        ->get();
        return view('usuario.consulta', compact('menuAtivo', 'consulta', 'search')); // Passando $search
    ...
    public function showProfile($id = null) {
        ...
        */
        ...
        return view('usuario.profile', compact('menuAtivo', 'user', 'perfis', 'oms', 'postos', 'id', 'ufs', 'cidades', 'situacoes', 'forcas', 'nivels', 'hoje', 'min', 'docFrente', 'docVerso'));
    }
    ...
    public function editarSenha(Request $request) {
        ...
        //$this->saveLog(Config::get('constants.msgLog.senhaAlterada'), Route::currentRouteName(), null, null, null);                            
    ...
    public function verDados($id){
        ...
        */
        ...
        $docFrente = $documentoService->obterFrente($user);
        $docVerso = $documentoService->obterVerso($user); 
        return view('usuario.verdados', compact('user', 'perfis', 'oms', 'postos', 'forcas', 'ufs', 'cidades', 'situacoes', 'roles', 'id', 'nivels', 'hoje', 'motivos','docFrente', 'docVerso'));
    }
...
$docVerso = $documentoService->obterVerso($user);
        ...
        return view('usuario.verdados_aguardando', compact('user', 'perfis', 'oms', 'postos', 'forcas', 'ufs', 'cidades', 'situacoes', 'roles', 'id', 'nivels', 'docFrente', 'docVerso'));
    }
    ...
    public function ListaInativos(Request $request) {
        $search = $request->get('search');

        // Iniciamos a query filtrando apenas os inativos (status 2)
        $consulta = User::where('status', 2)->with('motivoinativos');
...
public function usuariosNegados(Request $request) {
    $search = $request->get('search');
    $dataAtual = Carbon::now()->locale('pt_BR');

    // Iniciamos a query filtrando apenas os negados (status 6)
    $consulta = \App\User::where('status', 6);
    ...
    return view('pedido.pedidonegado', compact('consulta', 'dataAtual'));
}
...
public function precadastroLista(Request $request) {
    ...
    // Altere o número do status para o que representa 'pré-cadastro' no seu banco
    $consulta = \App\User::where('status', '5'); 
    ...
    // Verifique se o nome da view é exatamente este
    return view('usuario.listaPreCadastro', compact('consulta', 'dataAtual'));
}
...
public function documentosVencidos(Request $request) {
    ...
    // Buscamos apenas as colunas essenciais para a tabela. 
    // Isso evita carregar fotos ou documentos pesados na memória.
    $query = \App\User::select('id', 'name', 'cpf', 'validade')
                      ->where('status', '1')
                      ->whereDate('validade', '<', $hoje);
    ...
    if ($search) {
        ...
        if (is_numeric($cleanSearch) && strlen($cleanSearch) == 11) {
            ...
            // Usamos LIKE simples se o COLLATE estiver lento
            $query->where('name', 'LIKE', "%{$search}%");
        }
    }

    // Carrega todos os registros de uma vez, mas de forma otimizada
    $consulta = $query->get();
...
public function verDocumento($id, $doc, $tipo)
{
    ...
    public function mecenas(Request $request)
{
