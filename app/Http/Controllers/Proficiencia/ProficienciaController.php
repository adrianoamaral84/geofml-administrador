...
namespace App\Http\Controllers\Proficiencia;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use \Crypt;
use \App\Proficiencia;
...
class ProficienciaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $consulta = \App\Proficiencia::listAll($search);

    	//$consulta = \App\SituacaoCandidato::paginate(10);
        $menuAtivo = "administracao";

    	//dd($consulta);
    	return view('proficiencia.index', compact('menuAtivo','search','consulta'));
    }

    public function showFormCadastra()
    {
        $menuAtivo = "administracao";
    	return view('proficiencia.create', compact('menuAtivo'));
    }
    ...
    {	
        ...
        $menuAtivo = "administracao";
        $consulta = \App\Proficiencia::where('descricao', $validatedData['nome'])->count();
        if($consulta == 1){

        \Session::flash('message', ['msg'=>'Existe registro com esse nome!', 'class'=>'danger']);
        return redirect()->route('proficiencia.create.show', compact('menuAtivo'));
        }
        $proficiencia = new Proficiencia;  
    	//$uf->sigla = $validatedData['sigla'];   	 	
    	$proficiencia->descricao = $validatedData['nome'];
    	$proficiencia->save();
        //$menuAtivo = "administracao";

    	//dd($area);
    	\Session::flash('message', ['msg'=>'Cadastrado com sucesso!', 'class'=>'success']);
        return redirect()->route('proficiencia.index', compact('menuAtivo'));
    }
    ...
    {   
        ...
        $id = Crypt::decrypt($id);        
        $consulta = \App\Proficiencia::find($id);
        $menuAtivo = "administracao";
        //dd($consulta);
        return view('proficiencia.edit', compact('menuAtivo','consulta'));
    }
    public function update(Request $request)
    {
        ...
        $consulta = \App\Proficiencia::find($request->id);     
