...
public function liberar(Request $request){
        ...
        $dt1_1 = date("Y-m-d", strtotime(str_replace("/","-",$validatedData['peridoinicial1'])));
        $dt1_12 = date("m", strtotime(str_replace("/","-",$validatedData['peridoinicial1'])));
        $dt1_2 = date("Y-m-d", strtotime(str_replace("/","-",$validatedData['final1'])));
        ...
        $AnoEntrada = date("Y", strtotime($dt1_1));
        $MesAnoSaida = date("m", strtotime($dt1_2));

        $ProximoMesEntrada = date("Y-m-d", strtotime(date("Y-m-d",strtotime($dt1_1))."+1 month"));
        $ProximoAnoEntrada = date("Y", strtotime($dt1_1));
        
        $buscaHospedesMes = \App\Hospede::whereMonth('data_inicio',$mesAtual)
        ->whereYear('data_inicio', $AnoEntrada)
        ->whereIn('status', [2,3,4,5])
        ->where('und_habitacionais_id',$request->unidadeshabitacionais)
        ->whereNull('checkout_at')
        ->get();

        $buscaHospedesProximoMes = \App\Hospede::whereMonth('data_inicio',$proximoMes)
        ->whereYear('data_inicio', $AnoEntrada)
        ->whereIn('status', [2,3,4,5])
        ->where('und_habitacionais_id',$request->unidadeshabitacionais)
        ->whereNull('checkout_at')
        ...
        $countbuscaHospedeMes = count($buscaHospedesMes);
        $countbuscaHospedeProximoMes = count($buscaHospedesProximoMes);

        $dt1_1 = strtotime($dt1_1);
        $dt1_2 = strtotime($dt1_2);

        if($countbuscaHospedeMes >= 1){


            foreach ($buscaHospedesMes as $key => $value){
            
            
            $dt2_1 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($value->data_inicio))."+1 days"));
            $dt2_1 = strtotime($dt2_1);

            $dt2_2 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($value->data_termino))."-1 days"));
            $dt2_2 = strtotime($dt2_2);
            
            
            if( ($dt1_1 <= $dt2_2) && ($dt1_2 >= $dt2_1) ){
               

            if($value->id <> $request->id1) {  
                \Session::flash('message', ['msg'=>"Existe registro nesta data!", 'class'=>'danger']);
        ...
        ///////////////////////////////////////////////////////////////////////
        if($countbuscaHospedeProximoMes >= 1){


            foreach ($buscaHospedesProximoMes as $key => $values){
            
            //dd($values);
            $dt2_1 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($values->data_inicio))."+1 days"));
            $dt2_2 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($values->data_termino))));
        ...
        $hospedagem = \App\Hospede::with('user')->findOrFail($request->id1);
        // Aplica  desconto do usuário, caso exista
        $totalValor = $hospedagem->user->aplicarDesconto($totalValor);
    ...
    public function liberar_OLD(Request $request){

        //dd($request->unidadeshabitacionais);
        date_default_timezone_set('America/Sao_Paulo');
        $tipoUND = \App\UnidadeHabitacional::where('id', $request->unidadeshabitacionais)->first();
        $tipoUND = $tipoUND->tipo_und_hab_id;
        ...
        //$d = new \DateTime($validatedData['peridoinicial1']);
        ...
        //dd($mesAtual);
        //$month_start = Carbon::parse($validatedData['peridoinicial1']);
        //$date = Carbon::now(); // 2019-03-12
        //$aa = $month_start->addMonth(1)->format('Y-m-d');
        //dd($aa);
        //$next_month = $d->add(new \DateInterval('P1M'));
        //$MesAnoEntrada = date("m", strtotime($dt1_1));
        //$aaa = strtotime($dt1_1);
        //dd($aaa);
        //$final = date("Y-m-d", strtotime("+1 month", $aaa));
        //dd($dt1_1);
        $AnoEntrada = date("Y", strtotime($dt1_1));
        $MesAnoSaida = date("m", strtotime($dt1_2));
        ...
        //$ProximoMesEntrada = 2;
        //dd($dt1_1);
        //$dt2_1 = date("Y-m-d", strtotime(str_replace("/","-","06/10/2024")));
        //$dt2_1 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($dt2_1))."+1 days"));
        //$dt2_2 = date("Y-m-d", strtotime(str_replace("/","-","11/10/2024")));
        //$dt2_2 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($dt2_2))."-1 days"));
        //dd($dt2_2);
        ...
        if($countbuscaHospedeProximoMes >= 1){
            ...
            $dt2_1 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($values->data_inicio))."+1 days"));
            $dt2_2 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($values->data_termino))));
            //$dt2_2 = date('Y-m-d', strtotime(date("Y-m-d",strtotime($values->data_termino))."-1 days"));
            //dd($dt1_1);
        ...
        //dd($value);
        $VerificaHospedagemEntrada = \App\Hospede::
        ...
        $VerificaHospedagemSaida = \App\Hospede::
        where('data_termino', $value)
        ->where('und_habitacionais_id', $request->unidadeshabitacionais)
        ->where('status', 2)
        ->orderBy('data_inicio')
        ->count();
        
        $CountVerificaHospedagem[] = $VerificaHospedagemEntrada;
        $CountVerificaHospedagemSaida[] = $VerificaHospedagemSaida;
        }
        
        dd($CountVerificaHospedagemSaida);
        if(in_array('1', $CountVerificaHospedagemSaida)){
            dd('contem');
        }
    ...
    public function verdados($id){
        ...
        $dataInicioAnoPassado = $AnoPassado."-12-25";
        //$dataInicioAnoPassado = strtotime($dataInicioAnoPassado);

        $dataFinalAnoPassado = $AnoAtual."-01-05";
        //$dataFinalAnoPassado = strtotime($dataFinalAnoPassado);

        $hospedagensAnoPassado = \App\Hospede::where('user_id', $hospedagem->user_id)
        ->whereBetween('data_inicio', [$dataInicioAnoPassado, $dataFinalAnoPassado])
        ->WhereNotNull('und_habitacionais_id')
        ->WhereNotNull('checkout_at')
        ->with('user')
        ->get();

        if(count($hospedagensAnoPassado) > 0){
            $contemplado = 1;   
        }else{
            $contemplado = 0;
        }

        $temporada = \App\Temporada::where('id', 2)->first();

        $ContempladoNessaTemporada = \App\Hospede::where('user_id', $hospedagem->user_id)
        ->whereBetween('data_inicio', [$temporada->data_inicio, $temporada->data_termino])
        ->where('status', 2)
        ->WhereNotNull('und_habitacionais_id')
        //->WhereNotNull('checkout_at')
        ->with('user')
        ...
        //dd($ContempladoNessaTemporada);

        if(count($ContempladoNessaTemporada) > 0){
              $contemplado = 1;
        }else{
     ...
     public function verdados_aguardando_liberacao($id){
        ...
        $id = Crypt::decrypt($id);
        $hospedagem = \App\Hospede::where('id', $id)->with('user')->first();
        $ultimoPagamento = \App\Pagamento::where('hospedagem_id', $hospedagem->id)
        ->orderBy('id', 'desc')
        ->first();

        if ($ultimoPagamento) {
            $this->atualizarSituacaoPagamento($ultimoPagamento);
            $ultimoPagamento->refresh();
        }
    ...
    private function atualizarSituacaoPagamento($pagamento)
{
    ...
    if (
    in_array($pagamento->situacao, ['CONCLUIDO', 'PAGO', 'PAGAMENTO_CONCLUIDO'])
    && $pagamento->aplicado_hospedagem == 0
) {
    $hospedagem = \App\Hospede::find($pagamento->hospedagem_id);

    if ($hospedagem) {

        $valorPagamento = $pagamento->valor ?? 0;

        $hospedagem->valor_pago = $valorPagamento;

        $hospedagem->valor_restante = max(
            0,
            ($hospedagem->valor ?? 0) - $valorPagamento
        );

        $hospedagem->save();

        $pagamento->aplicado_hospedagem = 1;
        $pagamento->save();
    }
}
