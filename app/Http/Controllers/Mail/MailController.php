...
class MailController extends Controller
{
    ...
    public function enviaMailEspera($id)
    {
        
        //dd(session('link'));
        $id = Crypt::decrypt($id);
        $hospedagem = \App\Hospede::findOrFail($id);
        $user = \App\User::findOrFail($hospedagem->user_id);
        $hospedagem->status = 7;
        $hospedagem->update();
        //dd($user);
        \Illuminate\Support\Facades\Mail::queue(new \App\Mail\enviaMensagemEspera($hospedagem));
/*
        try{
            //Mail::send
            $email = \Illuminate\Support\Facades\Mail::queue(new \App\Mail\enviaMensagemEspera($hospedagem));
        }catch(\Exception $e){
            \Log::error($e->getMessage()); //armazenar o erro no log
  ...
  */      
        ...
        \Session::flash('message', ['msg'=>'E-mail Enviado ao Usuário!', 'class'=>'success']);
    ...
    public function enviaMailDataDocumentoVencido($id)
    {
        

        $id = Crypt::decrypt($id);
        $user = \App\User::findOrFail($id);
        \Illuminate\Support\Facades\Mail::queue(new \App\Mail\ValidadeDocumento($user));
        \Session::flash('message', ['msg'=>'E-mail enviado com sucesso!', 'class'=>'success']);
        return redirect()->back();
