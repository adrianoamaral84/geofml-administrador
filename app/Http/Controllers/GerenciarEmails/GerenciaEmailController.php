...
class GerenciaEmailController extends Controller
{
    ...
    public function index(){
	    ...
	    return view('emails.index', compact('consulta', 'assinatura'));
	...
	public function atualizar(Request $request){
        ...
        date_default_timezone_set('America/Sao_Paulo');
            ...
            'corpo.max' => 'Campo Mensagem deve ter no max 65535 caracteres',
            'corpo.min' => 'Campo Mensagem deve ter no min 2 caracteres',
            'corpo.required' => 'Campo obrigatório',        

            'cabecalho.max' => 'Campo Mensagem deve ter no max 255 caracteres',
            'cabecalho.min' => 'Campo Mensagem deve ter no min 2 caracteres',
        ...
        $validatedData = [
            ...
            'cabecalho' => 'required|min:5  |max:65535',
            'corpo' => 'required|min:10|max:65535',
            'assunto' => 'required|min:10|max:255',
        ];
        $validatedData = $request->validate($validatedData, $customMessages);

        $email = GerenciarEmails::where('id', $request->id)->first();
        //dd($email);
        $email->cabecalho = $validatedData['cabecalho'];
