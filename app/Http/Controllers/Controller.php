...
public function verificarCPFCadastradoAtualizaCPF($value, $id){
      	
	$id = Crypt::decrypt($id);
	//dd($value);
	$pegaCPF = User::where('id', $id)->first();
	//dd($pegaCPF->cpf);
        
	if (Auth::check()) {
	
	$teste = User::where('cpf', $value)
	->where('id', '!=', $id)
	->count();
	
	//dd($teste);
        if(User::where('cpf', $value)->where('id', '!=', $id)->count() >= 1){
		$count = 1;
		//dd('tem outro usuario com esse cpf');
	}else{
		$count = 0;
		//dd('nao tem outro cpf usando');
	}
	...
	//dd($count);
		...
		//dd('ok1');
            ...
            $counti = User::where('cpf', $value)->get();
