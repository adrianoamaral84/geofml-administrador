...
// --- TRAVA PARA IMPEDIR DATAS RETROATIVAS (NOVA INSCRIÇÃO) ---
        $hoje_real = date("Y-m-d");
        if (isset($minDate)) {
            // Se a data calculada for menor que hoje, força para hoje
            if (strtotime($minDate) < strtotime($hoje_real)) {
                $minDate = $hoje_real;
            }
        } else {
            $minDate = $hoje_real;
...
public function solicitarinscricaoEditNOVO($id){
    ...
    if($usuarioAutenticado->indeterminado != 1){
        if(isset($usuarioAutenticado->validade)) {
            if(strtotime($usuarioAutenticado->validade) < strtotime($hoje_data)){
                \Session::flash('message', ['msg'=>"Seu documento de identidade está com a data de validade vencida! Favor atualizar o documento para prosseguir", 'class'=>'danger']);
                return redirect()->route('home');
            }
        }        
    ...
    foreach ($GrupoTarifaPostoGraduacao as $value) {
        $tipoHab = \App\GrupoTarifa::where('id', $value->grupotarifa_id)->with('tipoundhabitacao')->get();
        foreach ($tipoHab as $values) {
            $unidades[] = $values->tipoundhabitacao;    
        }
    }

    // Configurações de Temporada e Bloqueio
    $altaTemporada = \App\Temporada::where('tipo_temporada_id', 1)->first();
    $data_inicio_alta = $altaTemporada->data_inicio;
    $data_termino_alta = $altaTemporada->data_termino;
    $diaBloqueado = \App\BloqueioDia::where('id', 1)->first();
    ...
    // Lógica original de definição de minDate e maxDate baseada no mês
    $minDate = \Carbon\Carbon::today()->format('Y-m-d'); // Default hoje
    $maxDate = \Carbon\Carbon::today()->addMonths(3)->format('Y-m-t');

    if($mesAtual == 12){
        if($hoje <= $diaBloqueado->dia){               
            $minDate = \Carbon\Carbon::today()->addMonths(1)->format('Y-m-01');
            $maxDate = \Carbon\Carbon::today()->addMonths(2)->format('Y-m-'.$diaBloqueado->limitedia);
        } else {
            $minDate = \Carbon\Carbon::today()->addMonths(2)->format('Y-m-01');
            $maxDate = \Carbon\Carbon::today()->addMonths(3)->format('Y-m-'.$diaBloqueado->limitedia);
        }  
    } elseif($mesAtual == 11){
        if($hoje <= $diaBloqueado->dia){               
            $minDate = \Carbon\Carbon::today()->format('Y-m-01');            
            $maxDate = \Carbon\Carbon::today()->addMonths(2)->format('Y-m-'.$diaBloqueado->limitedia);
        } else {
            $minDate = \Carbon\Carbon::today()->format('Y-m-d');            
            $maxDate = \Carbon\Carbon::today()->addMonths(3)->format('Y-m-'.$diaBloqueado->limitedia);
        }
    } elseif(in_array($mesAtual, ["03","04","05","06","07","08","09","10"])){
        if($hoje <= $diaBloqueado->dia){                               
            $minDate = \Carbon\Carbon::today()->format('Y-m-01');            
            $maxDate = \Carbon\Carbon::today()->addMonths(2)->format('Y-m-'.$diaBloqueado->limitedia);
        } else {             
    ...
    } elseif($mesAtual == "01" || $mesAtual == "02"){
        if($hoje <= $diaBloqueado->dia){               
            $minDate = \Carbon\Carbon::today()->addMonths(1)->format('Y-m-01');
            $maxDate = \Carbon\Carbon::today()->addMonths(2)->format('Y-m-'.$diaBloqueado->limitedia);
        } else {
    ...
    // --- SEGURANÇA EXTRA: Nunca permitir data anterior a HOJE ---
    if(strtotime($minDate) < strtotime($hoje_data)){
        $minDate = $hoje_data;
    }
    ...
    $a = [];
    $bloquearDias = \App\LockDays::where('tipo', 1)->get();
    foreach ($bloquearDias as $value) {
        $a[] = [$value->data_inicio, $value->data_fim];
    }
    
    $bloquearDias2 = \App\LockDays::where('tipo', 2)->get();
    foreach ($bloquearDias2 as $value1) {
       $a[] = $value1->data_inicio;
    }
    $a = json_encode($a);

    $horario = \App\Horario::all()->first();
    $hospedagem = \App\Hospede::where('id', $id_decrypt)->first();
    
    // Variável 'consulta' para evitar o erro Undefined Variable no Blade
    $consulta = $hospedagem;
    
    $peridoinicial = \Carbon\Carbon::parse($hospedagem->data_inicio)->format('d-m-Y')." - ".\Carbon\Carbon::parse($hospedagem->data_termino)->format('d-m-Y');

    // Retornando para a view 'hospede.edit_inscricao' (ajuste o caminho se necessário)
    return view('hospedagem.edit_inscricao', compact('consulta', 'peridoinicial', 'minDate', 'maxDate', 'hoje_data', 'a', 'horario', 'diaBloqueado', 'hospedagem', 'unidades'));
}
 ...
 // MES 12
    ...
    /*public function storeEdit(Request $request){
        ...
        // --- TRAVA DE SEGURANÇA CONTRA DATAS RETROATIVAS ---
    if ($request->periodo) {
        $datas = explode(' - ', $request->periodo);
        // Converte DD-MM-YYYY para YYYY-MM-DD
        $data_inicio_selecionada = \Carbon\Carbon::createFromFormat('d-m-Y', trim($datas[0]))->format('Y-m-d');
        $hoje = date('Y-m-d');

        if ($data_inicio_selecionada < $hoje) {
            \Session::flash('message', ['msg' => "Erro: Não é permitido selecionar datas retroativas!", 'class' => 'danger']);
            return redirect()->back();
        }
    }
    // --- FIM DA TRAVA ---
 ...
 /*public function storeEdit(Request $request)
{
    ...
    // 1. Tenta encontrar a hospedagem antes de qualquer coisa para evitar erro de variável nula
    $id = $request->id;
    $consulta = \App\Hospede::find($id);

    if (!$consulta) {
        \Session::flash('message', ['msg' => "Registro não encontrado!", 'class' => 'danger']);
        return redirect()->route('hospede.meuspedidos');
    }

    // 2. TRAVA DE SEGURANÇA CONTRA DATAS RETROATIVAS
    if ($request->periodo) {
        try {
            $datas = explode(' - ', $request->periodo);
            $data_inicio_selecionada = \Carbon\Carbon::createFromFormat('d-m-Y', trim($datas[0]))->startOfDay();
            $hoje = \Carbon\Carbon::now()->startOfDay();

            if ($data_inicio_selecionada->lt($hoje)) {
                \Session::flash('message', ['msg' => "Erro: Não é permitido selecionar datas retroativas!", 'class' => 'danger']);
                // Passamos o objeto consulta de volta para evitar o erro "Undefined variable"
                return redirect()->back()->withInput()->with('consulta', $consulta);
            }

            $inicial = $data_inicio_selecionada->format('Y-m-d');
            $final = \Carbon\Carbon::createFromFormat('d-m-Y', trim($datas[1]))->format('Y-m-d');
            
        } catch (\Exception $e) {
            \Session::flash('message', ['msg' => "Formato de data inválido!", 'class' => 'danger']);
            return redirect()->back()->withInput()->with('consulta', $consulta);
        }
    }

    // 3. CÁLCULOS E DESCRIPTOGRAFIA
    try {
        $totalValor = Crypt::decrypt($request->cod);
    } catch (\Exception $e) {
        $totalValor = $request->valortarifa; 
    }
    ...
    // 4. ATRIBUIÇÃO E SALVAMENTO
    $consulta->user_cpf = Auth::user()->cpf;
    $consulta->user_id = Auth::user()->id;
    $consulta->tipo_und_id  = $request->tipo;
    $consulta->data_inicio = $inicial;
    $consulta->data_termino = $final;
    $consulta->adulto = $request->adultos;
    $consulta->crianca  = $request->criancas;
    $consulta->pne  = $request->pne;
    $consulta->pet  = $request->pet;
    ...
    \Illuminate\Support\Facades\Mail::queue(new \App\Mail\ConfirmaHospedagem($consulta));
...
public function storeEdit(Request $request)
{
    ...
    // 1. Localiza a inscrição pelo ID (enviado via hidden input)
    $consulta = \App\Hospede::find($request->id);

    if (!$consulta) {
        \Session::flash('message', ['msg' => "Inscrição não encontrada!", 'class' => 'danger']);
        return redirect()->route('hospede.meuspedidos');
    }

    // 2. Processa as datas vindas do campo 'periodo' (Ex: "10-03-2024 - 15-03-2024")
    if ($request->periodo) {
        $datas = explode(' - ', $request->periodo);
        $inicial = \Carbon\Carbon::createFromFormat('d-m-Y', trim($datas[0]))->format('Y-m-d');
        $final = \Carbon\Carbon::createFromFormat('d-m-Y', trim($datas[1]))->format('Y-m-d');
        
        // Cálculo exato das diárias para o banco
        $diarias = \Carbon\Carbon::parse($inicial)->diffInDays(\Carbon\Carbon::parse($final));
        
        $consulta->data_inicio = $inicial;
        $consulta->data_termino = $final;
        $consulta->qntdiarias = $diarias;
    }
    ...
    // Mantém o valor original ou atualiza se o seu sistema recalcular na confirmação
    if($request->valortarifa) {
        $consulta->valortarifa = $request->valortarifa;
    }
   ...
   /* public function ConfirmarDados(Request $request){
    ...
    // --- TRAVA DE SEGURANÇA: IMPEDE AVANÇAR COM DATA RETROATIVA ---
    if ($request->periodo) {
        try {
            $datas = explode(' - ', $request->periodo);
            $data_inicio = \Carbon\Carbon::createFromFormat('d-m-Y', trim($datas[0]))->startOfDay();
            $hoje = \Carbon\Carbon::now()->startOfDay();

            if ($data_inicio->lt($hoje)) {
                \Session::flash('message', ['msg' => "Erro: Não é permitido selecionar datas anteriores a hoje!", 'class' => 'danger']);
    ...
    // 1. TRAVA DE SEGURANÇA E TRATAMENTO DA DATA
    if (!$request->periodo) {
        \Session::flash('message', ['msg' => "Por favor, selecione um período!", 'class' => 'danger']);
        return redirect()->back()->withInput();
    }

    try {
        $datas = explode(' - ', $request->periodo);
        $inicial_br = trim($datas[0]);
        $final_br = trim($datas[1]);
        
        $pi = \Carbon\Carbon::createFromFormat('d-m-Y', $inicial_br)->format('Y-m-d');    
        $pf = \Carbon\Carbon::createFromFormat('d-m-Y', $final_br)->format('Y-m-d');
        
        $data_inicio_validacao = \Carbon\Carbon::createFromFormat('Y-m-d', $pi)->startOfDay();
        $hoje = \Carbon\Carbon::now()->startOfDay();

        // Bloqueia se a data de início for no passado
        if ($data_inicio_validacao->lt($hoje)) {
            \Session::flash('message', ['msg' => "Erro: Não é permitido selecionar datas anteriores a hoje!", 'class' => 'danger']);
            return redirect()->back()->withInput();
        }
    } catch (\Exception $e) {
    ...
    // 2. CÁLCULO DE DIÁRIAS
    $datework = \Carbon\Carbon::createFromDate($pi);
    $diasHospedagem = $datework->diffInDays($pf);
    
    if($diasHospedagem == 0) $diasHospedagem = 1; // Garante pelo menos 1 diária

    // 3. DEFINIÇÃO DE TEMPORADAS
    $pegaTemporadaAlta = \App\Temporada::where('tipo_temporada_id', 1)->first();
    $pegaTemporadaBaixa = \App\Temporada::where('tipo_temporada_id', 2)->first();
    ...
    $data_loop = new \DateTime($pi);
    $diasAltaTemporada = 0;
    $diasBaixaTemporada = 0;

    for ($i=0; $i < $diasHospedagem; $i++) { 
        // Lógica: a cada iteração adiciona 1 dia para checar se cai em alta ou baixa
        if($i > 0) $data_loop->add(new \DateInterval('P1D'));
        $data_comparacao = $data_loop->format('Y-m-d');
        $timestamp_comparacao = strtotime($data_comparacao);
    
        if ($dataTemporadaAltaInicio <= $timestamp_comparacao && $timestamp_comparacao <= $dataTemporadaAltaTermino) {
            $diasAltaTemporada++;
        } elseif ($dataTemporadaBaixaInicio <= $timestamp_comparacao && $timestamp_comparacao <= $dataTemporadaBaixaTermino) {
            $diasBaixaTemporada++;
        } else {
            // Caso não esteja em nenhum intervalo, define como baixa por padrão ou alta conforme sua regra
            $diasBaixaTemporada++;
        }
    }
    ...
    $calculaDiariaAlta = $diasAltaTemporada * $tarifa->valor;
    $calculaDiariaBaixa = $diasBaixaTemporada * $tarifa->valor_baixa;
    $totalValorBruto = $calculaDiariaAlta + $calculaDiariaBaixa;

    $totalValor = Crypt::encrypt($totalValorBruto);
    $tipos = \App\TipoUndHab::all();
    
    // IMPORTANTE: Criamos o objeto $consulta para o Blade
    $consulta = $request;
